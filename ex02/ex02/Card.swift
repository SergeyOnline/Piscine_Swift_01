...
class Card: NSObject {
	...
	override var description: String {
		...
		if self.value.rawValue <= 10 && self.value.rawValue != 15 {
			res = "(\(self.value.rawValue), \(self.color.rawValue))"
		} else if (self.value.rawValue == 15) {
			res = "(\(self.value))"
		} else {
			res = "(\(self.value), \(self.color.rawValue))"
		}
